...
Đẩy insight tới người dùng qua kênh ngoài thay vì bắt họ mở dashboard: bản tin định kỳ Thứ Hai và
Thứ Năm, nhóm theo vai trò đăng ký, chọn tin bằng xếp hạng rồi lấy top-N cứng. Chống gửi trùng bằng
`delivery_log` cộng chốt chặn chu kỳ, render thuần template (không gọi AI).

Channel-neutral: engine nhận một `ChannelAdapter`; phần vận chuyển email nằm ở capability
`gmail-transport`.
