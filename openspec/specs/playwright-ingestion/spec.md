...
PlaywrightConnector cho phép hệ thống ingestion xử lý các trang web Single Page Application (SPA) — nơi nội dung được render bằng JavaScript và không thể lấy qua HTTP GET đơn giản. Connector dùng headless Chromium (via `sync_playwright()`) để render trang, extract danh sách link bài viết, rồi dùng `trafilatura` để parse nội dung từng bài.
