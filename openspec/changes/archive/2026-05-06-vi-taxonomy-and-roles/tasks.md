...
- [x] 1.7 Verify: chạy `run_analysis` với 1 document → output JSON có topics/summary tiếng Việt + affected_roles
...
- [x] 2.5 Verify: query bảng insights thấy columns `affected_roles` và `published_at`
...
- [x] 3.4 Verify: chạy full pipeline → insights mới có `affected_roles` populated và `published_at` có giá trị
...
- [x] 4.4 Verify: `GET /api/v1/insights?role=Engineering` → chỉ trả insights có role Engineering
- [x] 4.5 Verify: `GET /api/v1/insights?sort_by=published_at` → sorted đúng thứ tự
...
- [x] 5.5 Sửa `frontend/src/components/Layout.tsx` — header tiếng Việt
...
- [x] 5.8 Verify: mở browser → UI hiển thị tiếng Việt, insight cards có roles và thời gian publish

## 6. End-to-End Verification (P1)

- [x] 6.1 Chạy full pipeline: ingestion → analysis → verify insights mới có tiếng Việt + roles + published_at
- [x] 6.2 Gọi API với filter role → response đúng
- [x] 6.3 Gọi API với sort_by → thứ tự đúng
- [x] 6.4 Mở browser → verify UI tiếng Việt, impact badges đúng màu
