...
- [ ] **0.1 (P2)** Soạn ~15 kịch bản **"khám phá bằng chi tiết"**: câu hỏi mà định danh chỉ tồn tại trong
...
- [ ] **0.2 (P2)** Đo `_rank` hiện tại trên bộ đó (miễn phí, tất định).
      *DoD:* bảng recall@5 / recall@60 / hạng xấu nhất per‑ca, lưu vào `measurement.md`.
      *Dep:* 0.1
- [ ] **0.3 (P2)** **Quyết định ghi thành văn**: ≥ 30% ca có bài đúng ngoài top‑5 ⇒ tiếp tục. Dưới ngưỡng
      ⇒ **đóng change**, ghi lý do vào `proposal.md`, giữ bộ kịch bản lại làm mốc.
      *DoD:* một đoạn kết luận có số, không phải cảm nhận.
      *Dep:* 0.2
- [ ] **0.4 (P2)** Chốt phương án giữ `chat_rank_harness` miễn phí (design D4: A / B / C). **Chặn mọi task
      còn lại.**
...
- [ ] **1.1 (P2)** Migration bảng `document_chunks` + index HNSW cosine + index `insight_id`.
      *DoD:* `alembic upgrade head` / `downgrade -1` sạch trên DB rỗng và DB có dữ liệu.
      *Dep:* 0.4
- [ ] **1.2 (P2)** Model SQLAlchemy `DocumentChunk` + quan hệ tới `RawDocument`/`Insight`.
      *DoD:* quan hệ nạp được bằng `selectinload`, không N+1.
      *Dep:* 1.1

## AI / Pipeline

- [ ] **2.1 (P2)** `app/ai/chunking.py::split_content()` — cửa sổ 400–600 token, overlap ~15%, cắt ưu tiên
...
- [ ] **2.2 (P2)** Sinh chunk + embedding trong `AnalyzerService` khi publish (cạnh `_attach_embedding`);
      lỗi → WARNING, **không** chặn tạo insight.
      *DoD:* test: embed lỗi → insight vẫn tạo, `document_chunks` rỗng cho bài đó.
      *Dep:* 1.2, 2.1
- [ ] **2.3 (P2)** `app/scripts/chunk_documents.py` — backfill idempotent (chỉ đụng bài chưa có chunk) +
      `--redo`. Ghi vào CLAUDE.md phần lệnh.
      *DoD:* chạy hai lần liên tiếp → lần hai không đổi gì.
      *Dep:* 2.2
- [ ] **2.4 (P2)** `purge_expired` xoá chunk cùng lúc xoá `normalized_content`.
      *DoD:* test: purge một bài → chunk của bài đó biến mất.
      *Dep:* 1.2

## Backend — retrieval

- [ ] **3.1 (P2)** `retrieve_chunk_ranks(question_vector, limit)` trong repository: `ORDER BY embedding <=>`
      trên `document_chunks`, gộp về `insight_id` lấy **thứ hạng tốt nhất** (design D1).
      *DoD:* trả `dict[insight_id, rank]`; test trên DB seed nhỏ.
      *Dep:* 1.2, 0.4
- [ ] **3.2 (P2)** `_rank` nhận `chunk_ranks` và thêm số hạng RRF thứ ba; insight thiếu chunk **mượn**
      ...
      *Dep:* 3.1
- [ ] **3.3 (P2)** Nối vào `_answer_global`: chạy song song với `list_for_chat` và `_embed_question`
...
- [ ] **4.1 (P2)** Thực thi phương án D4 đã chốt ở 0.4 (tách `retrieve_candidates` / fixture đông lạnh /
      ...
      *Dep:* 3.2
- [ ] **4.2 (P2)** Land bộ kịch bản 0.1 vào `chat_scenarios.jsonl` (group `detail_discovery`) + sinh query
      vector; chốt lại baseline RS kèm lý do.
      *DoD:* recall@5 nhóm `detail_discovery` tăng rõ so với số đo 0.2; không nhóm nào tụt.
      *Dep:* 4.1
- [ ] **4.3 (P2)** `chat_answer_harness --live` — context đổi ⇒ câu trả lời đổi; chốt lại baseline.
      *DoD:* Faithfulness ≥ 0,95 **và** Citation Precision = 1,00.
      *Dep:* 3.3
- [ ] **4.4 (P2)** Test bất biến "chunk không phải đích citation": mọi mapping `n → nguồn` chỉ chứa insight.
      *DoD:* `tests/test_chunk_not_citable.py`.
      *Dep:* 3.3

## DevOps

- [ ] **5.1 (P2)** Đo dung lượng + thời gian: số chunk thật, MB vector, thời gian backfill toàn corpus,
