...
- **Chat Q&A (M8, change `chatbot-qa` 22/07/2026)**: `POST /api/v1/chat` nhận `{question, history, insight_id?}`. Có `insight_id` → chế độ per-insight (context = insight fields + toàn bộ `raw_documents.normalized_content`); không có → chế độ toàn cục (server lọc + **xếp hạng HAI TẦNG** rồi cắt **top-K** thành index nén, đưa vào 1 lần gọi — **KHÔNG function-calling**, đo 22/07: cả 179 tin = 19.126 token = 1 lượt gọi, rẻ và nhanh hơn tool loop).
  ...
  - **HAI endpoint, MỘT pipeline (change `chat-streaming-sse`, 27/07/2026)**: `POST /api/v1/chat` blocking (giữ nguyên — client cũ, test, eval harness ④) và `POST /api/v1/chat/stream` SSE. Khác biệt duy nhất là `ChatService.answer(..., emit=...)`: có `emit` thì pipeline phát thêm `status`/`token` dọc đường. **Đừng tách thành hai nhánh logic** — grounding/xếp hạng/fail‑closed/budget phải dùng chung một đoạn code, không thì hai lối ra trôi khỏi nhau trong im lặng.
    ...
    - **Mốc tiến trình: `key` là TẬP ĐÓNG, và đa dạng đến từ DỮ LIỆU chứ không từ đồng nghĩa** (change `chat-status-milestones`, 03/08/2026). Sự kiện `status` mang `key` ∈ `searching`·`ranked`·`pinned`·`reading`·`expanding`·`retrying`·`composing` (định nghĩa **một chỗ**: `STATUS_KEYS` trong `chat_service.py`, `_status()` nổ nếu key ngoài tập; schema đối chiếu ở `schemas/chat.py::ChatStatusEvent`). Widget phân biệt mốc bằng **`key`**, KHÔNG bằng `text` — `text` mang số liệu của lượt nên hai lần phát cùng một mốc luôn khác chuỗi, so chuỗi sẽ đẻ dòng trùng; và sửa câu chữ tiếng Việt sẽ âm thầm đổi hành vi render. `key` lạ ⇒ widget hiện như dòng mới, **không nuốt** (server mới + client cũ không được làm mất thông tin của nhau); `key` vắng (server cũ) ⇒ gộp về `LEGACY_STATUS_KEY` = đúng hành vi một-dòng-ghi-đè cũ.
      - ⛔ **ĐỪNG "cải thiện" bằng cách xoay vòng cách nói đồng nghĩa** — luật gốc ở `chat_service.py` (*phát từ mốc thật; nói sai việc còn tệ hơn nói chung chung*) vẫn nguyên hiệu lực. Mốc mới chỉ được thêm khi nó nói được điều **chỉ đúng cho lượt này**.
      - **Widget XẾP CHỒNG (trần 4 dòng), không ghi đè.** Hệ quả quan trọng: ghi chú cũ *"hai status cách nhau vài chục ms chỉ làm dòng chữ nhấp nháy"* **không còn đúng** — nhấp nháy là hệ quả của mô hình ghi-đè-một-dòng. Đo thật 03/08: `ranked` và `reading` cùng ở **2,29s** và đó là hành vi ĐÚNG.
      - **Đo thật 03/08/2026** (toàn cục, 179 tin, client đã làm ấm): `searching` **0,30s** → `ranked` **2,29s** (*"Đã lọc 179 tin, 58 tin khớp"*) → `reading` **2,29s** → token đầu **5,26s** → commit **6,78s**. Khoảng im lặng thật là **0,30→2,29s** (`gather(list_for_chat, embed→chunk)`): trước khi nó xong thì **không có số thật nào tồn tại** để nói. Không lấp bằng mốc bịa; ràng buộc đúng là *không khoảnh khắc nào giao diện không nói gì*, và dòng `searching` ở lại suốt khoảng đó.
      - **`retrying` đi từ thread của model ra luồng SSE** qua `ChatStreamState.on_retry` + `run_coroutine_threadsafe` (`_retry_notifier`). Lượt hỏi lại chống-cắt nằm **bên trong** `GeminiClient`, service chỉ giành lại quyền điều khiển sau khi nó xong — quá muộn để báo. `_notify_retry()` **nuốt mọi lỗi**: mốc tiến trình là trang trí, câu trả lời đã trả tiền để sinh ra thì không.
  ...
  - **TRA CỨU NGOÀI khi corpus thiếu dữ kiện — Fork B2** (change `chat-web-fallback`, 03/08/2026). Mặc định **TẮT** (`CHAT_WEB_FALLBACK_ENABLED=false`). Ca chữa: hỏi so sánh A với B mà corpus chỉ có A ⇒ cách cũ **từ chối toàn bộ**, vứt luôn vế trả lời được. Cơ chế: model phát sentinel mang tham số `[[TRA_CỨU_NGOÀI: <truy vấn>]]` **KÈM** phần trả lời được (khác `OUT_OF_SCOPE_SENTINEL` vốn phải là *toàn bộ* câu trả lời) → server tra cứu → trả lời lại. Trần bước **2 → 3**; bước tải trang KHÔNG tính là bước.
    - **Grounding chỉ lấy `uri`, nội dung do `trafilatura` tải.** ⚠️ ĐỪNG "tối ưu" bằng cách dùng thẳng bản tóm tắt model viết ở bước tra cứu (Fork B1): khi đó citation trỏ về trang gốc mà văn bản cạnh nó do model viết ⇒ một lần diễn giải sai thành **lời bịa có kèm nguồn hợp lệ**, đúng thứ mô hình trích dẫn sinh ra để chặn.
    - **Fork A (để model tự trả lời khi bật tool) đã LOẠI, có bằng chứng**: `types.GroundingChunkWeb.model_fields` = `['domain','title','uri']` — **không có snippet** (xác minh SDK 1.75.0). Context của ta không còn đầy đủ ⇒ `Faithfulness` mất cơ sở đo, và `grounding_supports` thành hệ trích dẫn thứ hai chạy song song với `[n]`.
    - **`grounding_chunks[].web.title` chỉ là TÊN MIỀN** (`'google.com'`) và **`.uri` là link chuyển hướng** `vertexaisearch.cloud.google.com/grounding-api-redirect/…`. Nên phải giải chuyển hướng — nhưng **vì TRÍCH DẪN, không vì nội dung**: đo 03/08, tỉ lệ lấy được nội dung y hệt nhau (8/9 cả hai cách), chỉ sau khi giải mới có URL bấm được + tiêu đề thật.
    - **Google Custom Search JSON API KHÔNG dùng được** — đóng với khách mới từ 2025, khách cũ dừng 01/01/2027. Đó là lý do không có đường "tự gọi search rồi tự đánh số".
    - **Nguồn web vào CÙNG dãy `[n]`, CÙNG bảng ánh xạ**; `Citation.kind` ∈ `insight`/`web` phân biệt LOẠI nguồn chứ không phải không gian số thứ hai. Frontend vẫn giải bằng đúng một `find(c => c.n === n)`. ⚠️ `_web_lookup_turn` trả **kèm mapping mới** — dãy số lượt cuối khác lượt đầu, giải citation bằng bảng cũ là trỏ sai.
    - **Bộ đếm RIÊNG `max_daily_web_searches`** (migration **015**, cột `chat_logs.web_searches`). Không gộp vào `max_daily_chat_calls`: $35/1.000 truy vấn so với ~$0,006 một câu trả lời. Bộ đếm phải ở DB — trần *tiền* mà đếm in-memory thì một vòng lặp restart là một vòng lặp tiêu tiền. Cạn quota ⇒ **không 429**, vẫn trả lời bằng corpus.
    - **Mode B KHÔNG BAO GIỜ đi đường web** — hai sentinel loại trừ nhau; `expanded` mới là chỗ phát sentinel tra cứu.
    - **Hiển thị Search Suggestions là YÊU CẦU TUÂN THỦ** điều khoản Google (`search_entry_point.rendered_content`), không phải hạng mục cắt được khi gấp.
    - ⚠️ **Bật cờ: `docker compose restart` KHÔNG đủ** — `env_file` nướng vào container lúc TẠO, phải `docker compose up -d backend`. Kiểm bằng `docker compose exec backend printenv | grep CHAT_WEB`, đừng tin `restart` đã ăn.
    - ⚠️ **Luật `_WEB_LOOKUP_RULE` phải GHI ĐÈ luật 2b tường minh.** Luật 2b (`CHAT_SYSTEM_PROMPT`: "trả lời vế có + nói rõ vế thiếu") có trọng số cao hơn prompt người dùng, nên nếu không nói rõ, model tuân 2b rồi **không phát sentinel** — tính năng bật mà không bao giờ chạy, không có gì đỏ ở đâu. Đây là lần thứ HAI trong cùng một change một luật thêm vào system prompt âm thầm vô hiệu hoá thứ khác (lần đầu: 2b phá `comparison_anaphora`).
    - ⚠️ **`tests/conftest.py` ghim cờ tính năng về mặc định shipped (`autouse`)** — bộ test KHÔNG được đọc `.env` của máy. Bật cờ trong `.env` từng làm **51 test đỏ**; chiều nguy hiểm hơn là *xanh giả*, khi hai người chạy hai bộ test khác nhau mà cùng tưởng đang gác một thứ. Thêm cờ mới ⇒ thêm vào `_SHIPPED_DEFAULTS`. Baseline của `chat_answer_harness` chốt ở **cờ TẮT**, và snapshot nay ghi lại cờ để chỗ lệch nhìn thấy được.
    - Đo 03/08: tỉ lệ `trafilatura` lấy được nội dung **8/9 = 89%** (lần đo đầu 3/6 = 50% là mẫu nhỏ dính `reddit.com`); grounding trả **9–10** nguồn nên `chat_web_max_sources` là cắt bắt buộc; `gemini-2.5-flash-lite` lái được tool. Số đo đầy đủ: `openspec/changes/chat-web-fallback/measurement.md`.
  - **Hội thoại LƯU BỀN THEO TAB** (change `chat-session-persistence`, 03/08/2026). Trước đó luồng chỉ sống trong `useState`, mà `history` lại do client dựng lại và gửi lên mỗi lượt ⇒ **một lần F5 xoá sạch** cả những lượt đã trả tiền cho model (server cố ý không lưu nội dung: `chat_logs` chỉ metadata). Nay `sessionStorage['radar-chat-v1']` giữ `{v, messages, workingSet, open}`; phủ F5, back/forward qua trang ngoài, khôi phục sau crash tab. Đóng tab thì mất — ranh giới có chủ đích.
    - ⚠️ **BẤT BIẾN: luồng hội thoại thuộc về TAB, KHÔNG thuộc về ngữ cảnh.** Khoá lưu trữ phải **PHẲNG** — không bao giờ nối scope / bài đang xem / working set vào khoá. `chat-context-isolation` (commit `8d5ffdb`) đã hỏng đúng kiểu này: `threads` đánh khoá theo scope nên rời bài là luồng biến khỏi màn hình. Lưu bền mở lại đúng cám dỗ đó dưới dạng "đánh khoá blob cho gọn", và triệu chứng mới sẽ là **bỏ hết chip = mất hội thoại**. `workingSet` là một TRƯỜNG bên trong blob, không phải một phần của khoá. Khoá bằng `ChatWidget.persistence.test.tsx::xoa_het_working_set_khong_dung_toi_hoi_thoai`.
    - **Lệch `SESSION_VERSION` ⇒ VỨT, không migrate.** Hình dạng `Citation` đổi liên tục (`n` thêm ở `chat-citation-integrity`, `kind` ở `chat-web-fallback`); blob cũ hồi sinh vào code mới sẽ giải marker `[n]` sang **tin khác** mà không có gì đỏ. Đổi hình dạng `Message`/`Citation`/`Ref` ⇒ tăng `SESSION_VERSION`.
    - **Lưu trọn `citations`, đặc biệt `insight_id`.** Rút gọn blob còn `{role, content}` cho "gọn" thì sau F5 cơ chế ghim của `chat-history-pinning` **tắt trong im lặng** — giao diện y hệt, người dùng chỉ thấy bot "quên" chuyện vừa bàn.
    - **KHÔNG lưu `searchSuggestions`**: nó là HTML đi thẳng vào `dangerouslySetInnerHTML` (hồi sinh từ storage = mở rộng ranh giới tin cậy), và là UI tuân thủ gắn với MỘT truy vấn tại MỘT thời điểm. **KHÔNG lưu `pending.text`**: `pending` cố ý vắng mặt trong deps của effect ghi — text tạm ở ca fail-closed là một câu hoàn toàn khác câu cuối.
    - **Lượt hỏi treo suy ra từ CẤU TRÚC** (message cuối có `role === 'user'` và không có `pending`), không từ một cờ lưu kèm. Kéo theo: `retryLast()` cắt luồng về **trước** lượt hỏi đó rồi để `send(question, base)` đặt lại bong bóng. Bản cũ lọc bong bóng lỗi rồi gọi `send()` vốn tự nối thêm lượt user ⇒ **câu hỏi hiện hai lần**; lỗi ẩn được vì đường lỗi hiếm, lượt gián đoạn biến nó thành đường thường.
    - ⚠️ **`src/test/setup.ts` xoá `sessionStorage` trước MỖI test** — các test trong cùng một file dùng chung một jsdom, không dọn thì widget khôi phục `open: true` của test trước và **24 test cũ đỏ** vì không còn nút "Mở trợ lý hỏi đáp" để bấm. Mỗi test là một tab mới.
  ...
  - **Benchmark xếp hạng là lưới DUY NHẤT bắt hồi quy `_rank`** (change `chat-rank-stability`, 27/07/2026). `_rank()` quyết định tin nào lọt vào index; cắt sai ở đây thì model **không bao giờ nhìn thấy** tin đúng mà **vẫn trả lời trôi chảy** từ phần còn lại — `chatbot-qa` 4b.2 đo được recall 42%, riêng câu "mô hình mã nguồn mở" còn 11%. Bốn file test chat cũ đều bảo vệ *cơ chế*, không file nào bảo vệ *chất lượng xếp hạng*.
    ...
    - **Kịch bản MANG ĐƯỢC lịch sử hội thoại** (`chat-followup-rewrite`, 31/07/2026): trường `turn1_question` + `turn1_cited`. Trước đó **0/98** kịch bản có `history` — không lưới nào canh đường hội thoại đa lượt, kể cả đường ghim của `chat-history-pinning`. Nhóm **`followup_new_topic`** (14 ca) đo *câu nối tiếp cần tin CHƯA từng bàn*; `load_scenarios` **NỔ** nếu `must_have ∩ turn1_cited ≠ ∅` — vì tin đã trích thì ghim đã lo, chấm recall cho nó là chấm một phép tính không tồn tại. Baseline 31/07: nhóm này recall@60 **1,000** · recall@5 **0,786** · hạng xấu nhất **29**; điểm dưới 1,00 ở đây là **mốc đo**, không phải hồi quy.
    - ⚠️ **Tổng recall@5 tụt 0,900 → 0,881 khi thêm nhóm này là do MẪU SỐ đổi** (71 → 85 câu có nhãn), không phải xếp hạng kém đi. So sánh đúng là **theo nhóm**: 0/83 kịch bản cũ đổi điểm.
  ...
  - **Query Reformulator — ĐÃ ĐO, ĐÃ BÁC BỎ** (change `chat-followup-rewrite`, 31/07/2026). Cơ chế đã được cài đủ (prompt viết lại + `rewrite_query()` trên flash-lite + số hạng RRF **thứ tư** trong `_rank`), đo bằng bản viết lại thật của model trên 14 kịch bản `followup_new_topic`, rồi **gỡ**: recall@5 `off` **0,79** → `add` **0,71** ▼. Nó nhấc được vùng GIỮA bảng (hạng 29 → 9, 21 → 11) nhưng **không chen được vào top-5**, mà lại đẩy một tin đang ở hạng 4 xuống 6. Số đo đầy đủ: `openspec/changes/archive/2026-08-03-chat-followup-rewrite/measurement.md` §11.
    - **Vì sao thất bại — đây mới là điều đáng nhớ**: baseline recall@**60** của nhóm này đã là **1,000**. Tin đúng **luôn nằm sẵn trong index**; vấn đề duy nhất là *thứ tự trong top-5*. Một số hạng RRF thứ tư trên bốn quá **tù** để dịch mép top-5 — RRF là phép trộn thô. Đó là hồ sơ năng lực của **reranker**, không phải của thêm-một-tín-hiệu-RRF. ⇒ Ai muốn chữa nhóm này tiếp thì đi hướng rerank (`C1` trong `chatbot_tobe_conformance.md`), mốc đo đã có sẵn.
    - ⚠️ **ĐÍNH CHÍNH hồ sơ**: bản trước của dòng này (và 3 proposal đã archive) ghi *"recall câu nối tiếp mù được chữa bằng bản gộp‑từ‑khoá tất định (0 gọi model)"*. **Nó chưa bao giờ tồn tại** — `_question_terms` có đúng hai caller, cả hai truyền `question` trần. Đó là một lời hứa ở phần "vì sao hoãn ⑦" rồi được các change sau trích dẫn lại như việc đã rồi. Cùng mẫu với "bất biến chỉ là lời hứa" của `chat-history-pinning`.
    - **`history` vẫn KHÔNG chạm truy hồi.** `_rank` chỉ nhận `question`; `chat-history-pinning` đi *vòng* qua `_rank` bằng cách tiêm thẳng id vào index chứ không dạy `_rank` đọc lịch sử. Đừng tưởng ngược lại.
    - ⚠️ **Bẫy đo đã trả giá**: `GeminiClient.embed_one()` mặc định `EMBED_TASK_DOCUMENT`, còn `ChatService._embed_question` và `build_fixture_chat` dùng `EMBED_TASK_QUERY`. Script đo tay quên truyền task_type sẽ đo một cặp query↔doc **lệch pipeline thật** mà không có gì báo — đo 31/07 lệch tới recall@5 0,643 vs 0,786 và hạng xấu nhất 72 vs 29. **Số đo tay phải đi qua cùng đường với production**, hoặc đo thẳng bằng harness.
